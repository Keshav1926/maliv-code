...
> **Model:** All semantic extraction uses Claude (claude-sonnet-4-6) subagents exclusively. Gemini and all other third-party backends are disabled. Do not check for `GEMINI_API_KEY`, `GOOGLE_API_KEY`, or any other provider key — always dispatch Claude subagents as described in Part B below.
...
**Step B2 - Dispatch subagents in batches of 5**

Call the Agent tool multiple times IN THE SAME RESPONSE — one call per chunk, up to **5 at a time**. Wait for all 5 to complete, then dispatch the next batch of up to 5, and so on. This is the only way they run in parallel within a batch. Never dispatch more than 5 subagents in a single message.

**IMPORTANT - subagent type:** Always use `subagent_type="general-purpose"`. Do NOT use `Explore` - it is read-only and cannot write chunk files to disk, which silently drops extraction results. General-purpose has Write and Bash access which the subagent needs.

Concrete example for 12 chunks (3 batches):
```
Batch 1 (one message, 5 Agent calls): chunks 1–5
→ wait for all 5 to finish
Batch 2 (one message, 5 Agent calls): chunks 6–10
→ wait for all 5 to finish
Batch 3 (one message, 2 Agent calls): chunks 11–12
→ wait for both to finish
```
Each batch is a single message with up to 5 Agent tool calls. Never mix batches into one message.
